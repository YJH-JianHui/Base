...
graph TB
    ...
    N -->|无| P[TenantLineInnerInterceptor<br/>租户拦截]
    O --> P
    P --> Q[执行SQL]
    Q --> R[返回结果]
    R --> S{是否有 @FieldFilter}
...
#### TenantLineInnerInterceptor（租户拦截器）
- **位置**：`com.baomidou.mybatisplus.extension.plugins.inner.TenantLineInnerInterceptor`
- **配置**：配合 `cn.kmdckj.base.config.TenantLineHandler` 使用
- **执行时机**：SQL执行之前
- **主要职责**：
...
sequenceDiagram
    ...
    participant MybatisT as TenantLineHandler
    participant DB as 数据库
...
sequenceDiagram
    ...
    participant TenantInt as TenantLineHandler
    participant DB as 数据库
...
### 3. TenantLineHandler（租户处理器）

**作用**：配合MyBatis-Plus的`TenantLineInnerInterceptor`，自动处理SQL中的租户条件
...
public boolean ignoreInsert(List<Column> columns, String tenantIdColumn) {
    ...
    // 2. 如果插入语句中已包含tenant_id，则不再自动添加
    return columns.stream()
            .anyMatch(c -> c.getColumnName().equalsIgnoreCase(tenantIdColumn));
}
```
...
-- 自动添加租户条件后（user和department都需要隔离）
